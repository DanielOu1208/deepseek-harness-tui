...
export class PromptController implements PromptControllerApi {
  ...
  constructor(private readonly deps: PromptControllerDependencies) {}
  ...
  installInteractions(): void {
    ...
    this.interactionDisposers.push(this.deps.ctx.on('approval/request', (request, next) => {
      ...
      if (agent === undefined || request.agent !== agent) return next()
  ...
  scheduleDraftSave(text: string): void {
    const agent = this.deps.getAgent()
    if (agent === undefined || this.deps.isClosing()) return
  ...
  async persistCurrentDraft(text?: string): Promise<void> {
    const agent = this.deps.getAgent()
  ...
  async restoreCurrentDraft(): Promise<void> {
    const agent = this.deps.getAgent()
    if (agent === undefined) return
    this.cancelDraftSave()
  ...
  private async attachImagePath(path: string): Promise<void> {
    const agent = this.deps.getAgent()
    if (agent === undefined) throw new Error('no active DeepSeek session')
  ...
  async chooseAttachments(path = ''): Promise<void> {
    if (path.trim() !== '') {
    ...
    if (choice.value === 'clear') this.clearPendingImages()
    else {
  ...
  async beginPrompt(text: string): Promise<string> {
    const agent = this.deps.getAgent()
    if (agent === undefined) throw new Error('no active DeepSeek session')
    const sessionId = String(agent.id)
    this.cancelDraftSave()
  ...
  async completePrompt(sessionId: string): Promise<void> {
    this.clearPendingImages()
    const nextDraft = this.deps.ui.getComposerText()
    if (nextDraft === '') await this.deps.draftStore.delete(sessionId)
    else await this.deps.draftStore.save(sessionId, nextDraft)
  }

  async recoverPrompt(sessionId: string, submittedText: string): Promise<void> {
  ...
  async promptMessage(text: string): Promise<UserMessage> {
    if (this.pendingImages.length === 0) {
    ...
    const refs = []
    for (const image of this.pendingImages) refs.push(await this.deps.ctx.attachments.saveImage(image))
  ...
  async confirmDiscardPendingImages(): Promise<boolean> {
