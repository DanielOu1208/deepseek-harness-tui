...
import type { DeepSeekTui, SettingsChoice } from './ui.js'
...
interface SettingDefinition {
  ...
  label: string
  description: string
  currentValue(controller: SettingsController): string
  apply(controller: SettingsController): Promise<void> | void
}
...
export const SETTINGS_PICKER_ITEMS: readonly PickerItem[] = SETTING_DEFINITIONS.map(
  ({ id, label, description }) => ({ value: id, label, description }),
)

function messageContainsImage(message: { content: readonly { type: string }[] }): boolean {
  return message.content.some(block => block.type === 'image')
}

export function assertModelSupportsSessionImages(
  agent: Agent,
  info: LlmResolvedModelInfo,
  hasPendingImages = false,
): void {
  if (info.inputModalities === undefined || info.inputModalities.includes('image')) return
  const messages = [
...
export class SettingsController {
  ...
  busyEnter: BusyEnter = 'queue'

  constructor(readonly deps: SettingsControllerDependencies) {
  ...
  async modelCompletions(prefix: string): Promise<PickerItem[]> {
  ...
  async choosePermission(): Promise<void> {
    ...
    if (choice !== undefined) await this.deps.runHarnessCommand(`/permission ${choice.value}`)
  }

  async loadModels(): Promise<ModelPickerSource[]> {
    ...
    for (const provider of this.deps.ctx.llm.listProviders()) {
      try {
        ...
        for (const model of models) available.push({ provider: provider.id, model: model.id, name: model.name })
      } catch {
  ...
  async selectModel(value: string): Promise<void> {
    ...
    if (interactive && info.reasoning !== undefined) {
      const reasoning = await this.deps.ui.choose(
        'Choose reasoning effort',
        reasoningPickerItems(info.reasoning),
        undefined,
        { initialValue: reasoningInitialValue(selection.current, ref) },
      )
  ...
  async loadReasoningPickerItems(): Promise<PickerItem[]> {
  ...
  async selectReasoning(value: string): Promise<void> {
    if (value.trim() === '') {
  ...
  async selectBusyEnter(value: string): Promise<void> {
    if (value.trim() === '') {
  ...
  async chooseSettings(action = ''): Promise<void> {
    if (action.trim() !== '') {
  ...
  async selectTranscriptDensity(): Promise<void> {
  ...
  private settingsChoices(): SettingsChoice[] {
  ...
  async saveModelDefault(): Promise<void> {
    const current = this.deps.getSelection()?.current
  ...
  async savePermissionDefault(): Promise<void> {
    const current = this.currentPermission()
  ...
  showRuntime(): void {
    const service = this.deps.ctx.get('pluginInventory') as unknown as { list(): PluginInventorySnapshot } | undefined
  ...
  async showSupport(): Promise<void> {
    const action = await this.deps.ui.choose(
  ...
  async editAdvancedSettings(namespace = ''): Promise<void> {
