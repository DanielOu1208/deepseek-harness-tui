...
import { healProfilesModuleFallback, initProfile, readProfileManifest, resolveProfileDir, writeProfileManifest, } from '@deepseek-ai/dsh-app-boot';
import { DEEPSEEK_API_KEY_REF, describeCredential, confirmCredentialLogout, openOfficialCredentialService, readMaskedSecret, } from './auth.js';
const PROFILE = 'tui';
...
`;
export function ensureTuiProfile(deps) {
    ...
    if (!Array.isArray(bundles))
        throw new Error(`deepseek: invalid tui profile manifest at ${join(dir, 'package.json')}`);
    const nextBundles = [
        BASE_BUNDLE,
...
async function runAuth(action, deps) {
...
export async function runLauncher(argv, deps) {
...
function packageManifestPath() {
    ...
    if (path === undefined)
        throw new Error('deepseek: cannot locate the launcher package manifest');
...
export function runOfficialDsh(args) {
    ...
    const forwarded = ['SIGINT', 'SIGTERM'].map(signal => {
    ...
    return new Promise((resolve, reject) => {
        ...
        child.once('close', (code, signal) => {
            for (const [name, handler] of forwarded)
