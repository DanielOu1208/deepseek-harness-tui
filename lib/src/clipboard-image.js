...
export function detectImageMediaType(data) {
    ...
    if (data.length >= 3 && data[0] === 0xff && data[1] === 0xd8 && data[2] === 0xff)
        return 'image/jpeg';
    const ascii = (start, length) => Buffer.from(data.subarray(start, start + length)).toString('ascii');
    if (data.length >= 6 && (ascii(0, 6) === 'GIF87a' || ascii(0, 6) === 'GIF89a'))
        return 'image/gif';
    if (data.length >= 12 && ascii(0, 4) === 'RIFF' && ascii(8, 4) === 'WEBP')
        return 'image/webp';
    return undefined;
}
async function defaultRunner(command, signal) {
...
function macCommand(outputFile) {
    const script = [
...
function windowsCommand(outputFile) {
...
function clipboardCommands(platform, outputFile) {
    if (platform === 'darwin')
        return [macCommand(outputFile)];
    if (platform === 'win32')
        return [windowsCommand(outputFile)];
