...
export class SessionInsightsController {
    ...
    async showActivity() {
        ...
        try {
            ...
            if (generation !== current.generation || current.agent !== agent)
                return;
    ...
    async exportSession(argument) {
        ...
        const presenter = {
            ...
            presentResult: (name, argumentsValue, result) => this.ctx.tools.get(name, agent)?.presentResult?.(argumentsValue, result),
        };
        const input = {
