export function reasoningInitialValue(current, next) {
...
export function questionPickerItems(options) {
...
export function questionLabelsFromValues(values, options) {
    return values.flatMap(value => {
        if (!value.startsWith('answer:') || value === OTHER_ANSWER_VALUE)
            return [];
...
export function stepReasoningEffort(reasoning, currentEffort, direction) {
    if (reasoning.efforts.length === 0)
        return { kind: 'unavailable', reason: 'no-efforts' };
    const effective = currentEffort ?? reasoning.defaultEffort;
    if (effective === undefined)
        return { kind: 'unavailable', reason: 'unknown-default' };
